...
MINIKB_ALLOWED_SCOPES = ["openid", "profile", "email"]
...
async def ensure_minikb_oauth_client(db: AsyncSession) -> None:
    result = await db.execute(select(AuthClient).where(AuthClient.client_id == MINIKB_CLIENT_ID))
    client = result.scalar_one_or_none()

    if client is None:
        db.add(
            AuthClient(
                client_id=MINIKB_CLIENT_ID,
                client_secret_hash=None,
                name=MINIKB_CLIENT_NAME,
                redirect_uris=_dump_list(MINIKB_REDIRECT_URIS),
                allowed_scopes=_dump_list(MINIKB_ALLOWED_SCOPES),
                pkce_required=True,
                status="active",
            )
        )
    ...
    merged_redirect_uris = list(dict.fromkeys([*redirect_uris, *MINIKB_REDIRECT_URIS]))
    if merged_redirect_uris != redirect_uris:
        redirect_uris = merged_redirect_uris
        changed = True

    normalized_scopes = list(dict.fromkeys([*allowed_scopes, *MINIKB_ALLOWED_SCOPES]))
    if normalized_scopes != allowed_scopes:
        allowed_scopes = normalized_scopes
        changed = True

    if client.name != MINIKB_CLIENT_NAME:
        client.name = MINIKB_CLIENT_NAME
        changed = True
